...
admin.add_view(ModelView(Detail, db.session, name="患者信息")) 
...
class Income_data():
    ...
    def saveData_csv(self, date, patient, cost1, cost2, cost3):
        arr = {'日期': date, '患者': patient, '金额': cost1, '金额': cost2, '金额': cost3}
        df = pd.DataFrame(arr, columns= ['日期', '患者', '金额', '金额', '金额'])
...
class OutComeData1():
    ...
    def OutcomeDaily(self):
        ...
        for x in db.session.query(Medicine.Price).filter(func.date(Medicine.time_get) == date.today()).all():
            print('yes')
            OutComeDictionary[now].append(int(str(x)[2:-3]))
    ...
    def OutcomeMonth(self):
        ...
        for i in db.session.query(Medicine.Price).filter(extract('year', Medicine.time_get)==year).filter(extract('month', Medicine.time_get)==month).all():
            print(int(str(i)[2:-3]))
            OutComeDictionaryMonthly[now].append(int(str(i)[2:-3]))
        print(OutComeDictionaryMonthly)
...
def add_patient_detail(subid): 
    ...
    if form.validate_on_submit():  
        ...
        if request.method == 'POST':
            ...
            arr['patient'].append(patient.name)
            arr['time'].append(now)
            df = pd.DataFrame(arr, columns= ['症状','患者','日期'])
